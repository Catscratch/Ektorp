...
public class StreamedCouchDbConnectorTest extends StdCouchDbConnectorTest {
  ...
  public void setUp() throws Exception {
    ...
    StdCouchDbInstance dbInstance = new StdCouchDbInstance(httpClient,
      ...
      public CouchDbConnector createConnector(String path, boolean createIfNotExists) {
        CouchDbConnector db = new StreamedCouchDbConnector(path, this,
            this.getObjectMapperFactory()) {
          @Override
  ...
  public void update() throws IOException {
    ...
    doAnswer(new MarshallEntityAndReturnAnswer(output,
        HttpResponseStub.valueOf(201, OK_RESPONSE_WITH_ID_AND_REV))).when(httpClient)
  ...
  public void create_should_post_if_id_is_missing() throws IOException {
    ...
    doAnswer(new MarshallEntityAndReturnAnswer(output,
        HttpResponseStub.valueOf(201, OK_RESPONSE_WITH_ID_AND_REV))).when(httpClient)
        .post(anyString(), any(HttpEntity.class));
    dbCon.create(td);
    ArgumentCaptor<HttpEntity> ac = ArgumentCaptor.forClass(HttpEntity.class);
  ...
  public void dates_should_be_serialized_in_ISO_8601_format() throws IOException {
    ...
    doReturn(HttpResponseStub.valueOf(201, output.toString("UTF-8"))).when(httpClient)
        .get("/test_db/some_id");
  ...
  public void throw_exception_when_in_conflict() {
    ...
    final ByteArrayOutputStream output = new ByteArrayOutputStream();
    doAnswer(new MarshallEntityAndReturnAnswer(output,
        ResponseOnFileStub.newInstance(409, "update_conflict.json"))).when(httpClient)
        .put(anyString(), any(HttpEntity.class));
    dbCon.update(td);
  }
  ...
  public void testPurge() {
    ...
    final ByteArrayOutputStream output = new ByteArrayOutputStream();
    doAnswer(new MarshallEntityAndReturnAnswer(output, HttpResponseStub.valueOf(200, rsp)))
        .when(httpClient).post(eq("/test_db/_purge"), any(HttpEntity.class));
