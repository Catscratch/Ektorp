...
public class BulkDocumentWriterTest {
  ...
  public void testCreateInputStreamWrapper() throws Exception {
    ByteArrayInputStream bis = new ByteArrayInputStream(
        "[{\"_id\":\"0\",\"key\":\"key_value\",\"value\":\"doc_value0\"},{\"_id\":\"1\",\"key\":\"key_value\",\"value\":\"doc_value1\"},{\"_id\":\"2\",\"key\":\"key_value\",\"value\":\"doc_value2\"}]"
            .getBytes("UTF-8"));
  ...
  public void testInputStreamBulkEntityIsEquivalentToInputStreamWrapper() throws Exception {
    ...
    InputStream inputStream = bw.createInputStreamWrapper(true,
        new ByteArrayInputStream(originalDocumentContent.getBytes("UTF-8")));

    ByteArrayOutputStream resultUsingInputStreamBulkEntity = new ByteArrayOutputStream();
    InputStreamBulkEntity inputStreamBulkEntity = new InputStreamBulkEntity(
        new ByteArrayInputStream(originalDocumentContent.getBytes("UTF-8")), true);
    inputStreamBulkEntity.writeTo(resultUsingInputStreamBulkEntity);

    InputStream inputStreamUsingInputStreamBulkEntity = new ByteArrayInputStream(
        resultUsingInputStreamBulkEntity.toByteArray());

    assertEquals(IOUtils.toString(inputStream),
        IOUtils.toString(inputStreamUsingInputStreamBulkEntity));
