...
public class BulkTest {
  ...
  public void setUp() {
    ...
    stdCouchDbConnector = new StdCouchDbConnector(databasePath, dbInstance,
        new StdObjectMapperFactory());
    streamedCouchDbConnector = new StreamedCouchDbConnector(databasePath, dbInstance,
        new StdObjectMapperFactory());
  ...
  public void doUpdateInBulkWithManyElements(CouchDbConnector db) {
    ...
    for (int i = 0; i < elementsCount; i++) {
      String currentId = "TestDocumentBean-" + i;
      TestDocumentBean bean = new TestDocumentBean(RandomStringUtils.randomAlphanumeric(32),
          RandomStringUtils.randomAlphanumeric(16), System.currentTimeMillis(), 0);
  ...
  public void doUpdateInBulkWithOneSmallInputStream(CouchDbConnector db) throws Exception {
    ...
    for (int i = 1; i <= iterationsCount; i++) {
      ...
      InputStream bulkDocumentAsInputStream = new ReaderInputStream(
          new StringReader("[" + mapper.writeValueAsString(doc) + "]"));
