...
public class SimpleViewGeneratorTest {
  ...
  public void views_should_be_generated_for_all_annotations() throws Exception {
    ...
    assertFalse("map function should be loaded from file in classpath",
        result.get("by_special2").getMap().startsWith("classpath:"));
    assertTrue("reduce function should be loaded from file in classpath",
        result.get("by_special2").getReduce().indexOf("keys") > -1);
    ...
    assertFalse("map function should be loaded from file in classpath",
        result.get("by_special3").getMap().startsWith("classpath:"));
  ...
  public void view_should_be_generated_for_fields_with_no_orderBy_specified() {
    Map<String, DesignDocument.View> result = gen
        .generateViewsFromPersistentType(SetRefDocWNoOrderBy.class);
    String expectedViewName = "ektorp_docrefs_children";
    assertTrue(result.containsKey(expectedViewName));
    assertEquals(expectedSetBasedDocrefMapFunctionWithNoOrderBy,
        result.get(expectedViewName).getMap());
  }

  @Test
  public void views_should_not_be_generated_if_view_is_specified() {
    Map<String, DesignDocument.View> result = gen
        .generateViewsFromPersistentType(RefDocWithViewSpecified.class);
    String expectedViewName = "ektorp_docrefs_children";
  ...
  public void given_a_discriminator_is_declared_on_getter_then_all_view_should_be_generated() {
    CouchDbConnector db = mock(CouchDbConnector.class);
    GenericRepository<DiscriminatorOnMethodType> repo = new CouchDbRepositorySupport<DiscriminatorOnMethodType>(
        DiscriminatorOnMethodType.class, db) {
      @GenerateView
      @Override
  ...
  public void given_a_discriminator_is_declared_on_child_type_then_docrefs_view_should_use_it() {
    ...
    DiscriminatingRepo repo = new DiscriminatingRepo(db);
    Map<String, DesignDocument.View> result = gen.generateViews(repo);
    assertEquals(expectedDocrefsFunctionWhereChildHasDiscriminator,
        result.get("ektorp_docrefs_children").getMap());
  }
  ...
  public void given_json_view_file_with_unix_line_endings_then_should_parse_successfully()
    ...
    DesignDocument.View view = generator.loadViewFromString(new ObjectMapper(),
        " {\"map\":\"function(doc) {\n  emit(doc._id, doc);\n  }\n\"}");
...
// ******************************* S U P P O R T   T Y P E S   B E L O W ********************************* //
  ...
  public static class CustomDiscriminatingRepo extends
      CouchDbRepositorySupport<DocWithCustomDiscriminator> {
  ...
  @TypeDiscriminator("doc.excoticField || doc.otherExcoticField")
  @SuppressFBWarnings("serial")
