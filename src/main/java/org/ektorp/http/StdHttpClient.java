...
public class StdHttpClient implements HttpClient {
  ...
  public static class Builder {
    ...
    /**
     * Controls if the http client should send Accept-Encoding: gzip,deflate header and handle
     * Content-Encoding responses. This enable compression on the server; although not supported
     * natively by CouchDB, you can use a reverse proxy, such as nginx, in front of CouchDB to
     * achieve this.
     * <p>
      ...
      if (enableSSL) {
        try {
          if (sslSocketFactory == null) {
            ...
            sslSocketFactory = relaxedSSLSettings ? new SSLSocketFactory(context,
    ...
    protected HttpParams configureHttpParams() {
      ...
      if (proxy != null) {
        params
            .setParameter(ConnRoutePNames.DEFAULT_PROXY, new HttpHost(proxy, proxyPort, protocol));
      }
      return params;
    }
    ...
    /**
     * If set to true, a monitor thread will be started that cleans up idle connections every 30
     * seconds.
     */
    ...
    /**
     * Bring your own Connection Manager. If this parameters is set, the parameters port,
     * maxConnections, connectionTimeout and socketTimeout are ignored.
     */
    ...
    /**
     * Set to true in order to enable SSL sockets. Note that the CouchDB host must be accessible
     * through a https:// path Default is false.
     */
    ...
    /**
     * Bring your own SSLSocketFactory. Note that schemeName must be also be configured to "https".
     * Will override any setting of relaxedSSLSettings.
     */
    ...
    /**
     * If set to true all SSL certificates and hosts will be trusted. This might be handy during
    ...
    /**
     * Activates 'Expect: 100-Continue' handshake with CouchDB. Using expect continue can reduce
     * stale connection problems for PUT / POST operations. body. Enabled by default.
     */
    ...
    public HttpClient build() {
      ...
      if (caching) {
        cachingHttpClient = WithCachingBuilder
            .withCaching(client, maxCacheEntries, maxObjectSizeBytes);
      }
