...
/**
 * A document's MIME multipart/related representation produced by CouchDB uses the order of the
 * attachments in the JSON _attachments object as the order of the attachments in the
 * multipart/related. Thus, the order must be preserved in order to parse a CouchDB
...
public class AttachmentsInOrderParser {
  ...
  /**
   * Parses a CouchDB document in the form of a JsonParser to get the attachments order. It is
   * important that the JsonParser come straight from the source document and not from an object, or
   ...
   */
  public static List<String> parseAttachmentNames(JsonParser documentJsonParser)
      throws IOException {
