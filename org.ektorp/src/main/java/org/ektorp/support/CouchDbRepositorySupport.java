 ...
 * <p>
 * Note that this class will try to access the standard design document named according to this
 * convention:
 * <p>
 ...
 * <p>
 * It is preferable that this design document must define a view named "all". The "all"-view should
 * only return document id's that refer to documents that can be loaded as this repository's handled
 * type.
 */
public class CouchDbRepositorySupport<T> implements GenericRepository<T> {
   ...
   */
  static final String AUTO_UPDATE_VIEW_ON_CHANGE = "org.ektorp.support.AutoUpdateViewOnChange";
  ...
  private CouchDbRepositorySupport(Class<T> type, CouchDbConnector db, boolean createIfNotExists) {
  ...
  /**
   * Alternative constructor allowing a custom design document name (not linked to the type class
   * name).
   */
  ...
  /**
   * If the repository's design document has a view named "all" it will be used to fetch all
   * documents of this repository's handled type.
   * <p>
   * "all" must return document ids that refers documents that are readable by this repository.
   * <p>
   * If the "all"-view is not defined, all documents in the database (except design documents) will
   * be fetched. In this case the database must only contain documents that are readable by this
   * repository.
   *
  ...
  /**
   * Allows subclasses to query views with simple String value keys and load the result as the
   * repository's handled type.
   * <p>
   ...
   * </p>
   * <p>
   * Will also generate view definitions for finder methods defined in this class and annotated by
   * the {@literal @}GenerateView annotation. The method name must adhere to the name convention of
   * findBy[Property].
   * </p>
   * <p>
   * The method:
   * </p>
   * <code>
   * {@literal @}GenerateView public List&lt;Sofa&gt; findByColor(String s) { return
   * queryView("by_color", s); }
   * </code>
   * <p>
  ...
  private void initDesignDocInternal(int invocations) {
    ...
    if (changed) {
      ...
      try {
        ...
        log.warn("Update conflict occurred when trying to update design document: {}",
            designDoc.getId());
        if (invocations == 0) {
          backOff();
          log.info("retrying initStandardDesignDocument for design document: {}",
              designDoc.getId());
