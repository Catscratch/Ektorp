...
public interface ReplicationTask extends ActiveTask {
  ...
  /**
   * @return the number of documents in the target DB which have been found to be either missing or
   * out of date. This number can be incremented more than once for the same document: for example
   * if a document is updated while a continuous replication rule is active. It won't however be
   * increased by more than 1 if a document is out of date by more than one revision: for example if
   * document X is at revision 3 in Source and revision 1 in Target, this number only increases by 1
   * when document X is checked.
   */
  long getTotalMissingRevisions();

  /**
   * @return the number of documents from the source DB whose presence and revision number in the
   * target DB have been checked. This number can be incremented more than once for the same
   * document: for example if a document is updated while a continuous replication rule is active,
   * it will be checked again. It won't however count multiple revisions of the same document each
   * time it's checked: for example if a document is at revision 3 the first time it's checked, this
   * count will still only increase by 1.
   */
  ...
  /**
   * Defines replication checkpoint interval in milliseconds. Replicator will requests from the
