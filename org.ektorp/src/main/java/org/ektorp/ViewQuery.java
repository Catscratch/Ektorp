...
public class ViewQuery {
  ...
  private static ObjectMapperFactory DEFAULT_OBJECT_MAPPER_FACTORY = new CachingObjectMapperFactory(
  ...
  /**
   * If set to true, the view query result will be cached and subsequent queries (with cacheOk set)
   * may be served from the cache instead of the db.
   * <p>
  ...
  /**
   * @param s will be parsed as JSON.
   * @return the view query for chained calls
   */
   ...
   * returned document has been deleted (in which case, it includes the "_deleted": true
   * attribute).
   *
