...
/**
 * Class for handling id and revision for persistent classes. A persistent class must either declare
 * setters and getters for the properties id and revision or annotate equivalent methods with
 * org.codehaus.jackson.annotate.JsonProperty("_id") and JsonProperty("_rev")
 * <p>
 * Map can be used as a document class, the map must contain the id field with key "_id" and
 * revision with the key "_rev".
 * <p>
 * org.codehaus.jackson.node.ObjectNode can olso be used as a document class.
 * <p>
 * For special needs, a custom document accessor can be registered through the method
 * registerAccessor(Class&lt;?&gt; documentType, DocumentAccessor accessor)
 */
public final class Documents {
  ...
  /**
   * Used to register a custom DocumentAccessor for a particular class. Any existing accessor for
   * the class will be overridden.
   */
  public static void registerAccessor(Class<?> documentType, DocumentAccessor accessor) {
    ...
    if (accessors.containsKey(documentType)) {
      ...
      LOG.warn(String
          .format("DocumentAccessor for class %s already exists: %s will be overridden by %s",
              documentType, existing.getClass(), accessor.getClass()));
    }
