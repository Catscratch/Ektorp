...
public class StdResponseHandler<T> implements ResponseCallback<T> {
  ...
  /**
   * Creates an DbAccessException which specific type is determined by the response code in the http
   * response.
   */
  ...
  protected static JsonNode responseBodyAsNode(InputStream inputStream, ObjectMapper mapper)
      throws IOException {
    return mapper.readTree(inputStream);
  }

  protected static <T> T checkResponseBodyOkAndReturnDefaultValue(HttpResponse hr, T defaultValue,
    ...
    try {
      ...
      throw new DbAccessException(
          "Unexpected response body content, expected {\"ok\":true}, got " + body.toString());
    } finally {
